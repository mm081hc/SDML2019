...
def main():
    ...
    trainer = Trainer(seq2seq, data_transformer, config.learning_rate, config.use_cuda, device=config.device,checkpoint_name="auto_encode_2gru_share_embedding_6epoch.pt")
    trainer.load_model()

    with open('./dataset/hw2.0_testing_data.txt', 'r') as testset:
        results = []
        seq_list = []
        i = 0
        for seq in testset:
    ...
    with open('./dataset/task1.txt', 'w') as f:
        for s in results:
            f.write(s + '\n')
