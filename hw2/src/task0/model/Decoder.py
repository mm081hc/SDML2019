...
class VanillaDecoder(nn.Module):
    ...
    def __init__(self, hidden_size, output_size, max_length, teacher_forcing_ratio, sos_id, use_cuda, device, embedding):
        ...
        self.gru = nn.GRU(hidden_size, hidden_size, num_layers=2, dropout=0.2)
    ...
    def forward(self, context_vector, targets):
        ...
        decoder_hidden = context_vector # [-1:] use last hidden layer
        ...
        if self.use_cuda:
            decoder_input = decoder_input.cuda(self.device)
            decoder_outputs = decoder_outputs.cuda(self.device)
