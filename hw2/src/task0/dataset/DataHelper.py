...
class Vocabulary(object):
    ...
    def build_vocab(self, data_path):
        ...
        with open(data_path, 'r', encoding='utf-8') as dataset:
            for seq in dataset:
                seq = seq.strip('\n')
                self.word_list.append(seq)
                seq = self.split_sequence(seq)  # seq: ['<SOS>', 'i',...,'<EOS>']
...
if __name__ == '__main__':
    ...
    test = "<SOS> tom seemed annoyed <EOS>"
    ...
    data_transformer = DataTransformer('train.txt', use_cuda=False)
